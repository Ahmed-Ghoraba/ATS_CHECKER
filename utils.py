...
def get_prompt_with_feedback(text, description, min_experience, max_experience, ats_criteria):
    return f"""
    You are an ATS (Applicant Tracking System) expert. 
    Evaluate the resume against the job description and minimum ATS score ({ats_criteria}%) and provide:

    1. The ATS score (number only, no % sign) on the first line.
    2. On the next lines, provide detailed FEEDBACK:
       - State clearly whether the resume is ACCEPTED or REJECTED based on the ATS score.
       - List what matched well (skills, experience).
       - List what is missing or weak.
       - Suggest improvements.

    Job Description (Experience {min_experience}-{max_experience} years):
    {description}
...
def get_ats_score_and_feedback(prompt, file_name):
    try:
        model = get_model()
        response = model.generate_content(prompt)
        
        if not response or not response.text:
            return None, "No response from ATS model."
        ...
        return None, "Error in ATS scoring."